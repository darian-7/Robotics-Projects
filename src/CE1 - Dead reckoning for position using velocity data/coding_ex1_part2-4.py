...
class OdometryNode(Node):
    ...
    def timer_callback_odom(self):
        ...
        Part 2: GPS measurements for robot position
        '''
        ...
        vr = (self.brspeed + self.brspeed)/2.0  # ... Your code here. Average right-wheels speed
        
        v = (vr + vl) / 2.0     # ... Linear velocity of the robot
        #w = (vr - vl) / self.l_wheels    # ... Angular velocity of the robot
        w = self.Gyro_yaw

        # Convert current lat and long to x & y using the initial points
        self.x, self.y = lonlat2xyz(self.lat, self.lon, lat0=40.0721778869625,lon0=-88.20980072021484)

        d_theta = w * dt      # Change in orientation

        self.theta += d_theta # ...Heading angle
